  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 font-mono text-xs">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-amber-500">🚔 STATION FLEET VEHICLES & TACTICAL ARMORY</h2>
          <p className="text-xs text-slate-400">Patrol vehicle dispatch, odometer mileage logs, armory weapon sign-out, & defect maintenance alerts.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {vehicles.map((v) => (
            <div key={v.plate} className="p-4 bg-slate-950 border border-slate-800 rounded space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-400 bg-slate-900/50 p-2 rounded border border-slate-800/50">
                ...
                <div className="col-span-2">Assigned Driver: <span className="text-amber-400 font-bold">{v.driver}</span></div>
              </div>
              {v.status === 'AVAILABLE' && (
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-lg space-y-4">
        ...
        <div className="space-y-3">
          {equipment.map((e) => (
            <div key={e.code} className="p-4 bg-slate-950 border border-slate-800 rounded flex justify-between items-center">
